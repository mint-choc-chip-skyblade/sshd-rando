...
class TrackerLocationLabel(QLabel):
    ...
    def __init__(self, location_: Location, search: Search, parent_area_button_) -> None:
        ...
        if self.location.name.startswith(self.parent_area_button.area):
            self.setText(self.location.name.replace(f"{self.parent_area_button.area} - ", ""))
        else:
    ...
    def update_color(self, search: Search) -> None:
        ...
        if self.location.marked:
            self.setStyleSheet(f"{TrackerLocationLabel.default_stylesheet} color: gray; text-decoration: line-through;")
        elif self.location in search.visited_locations:
            self.setStyleSheet(f"{TrackerLocationLabel.default_stylesheet} color: dodgerblue;")
        else:
