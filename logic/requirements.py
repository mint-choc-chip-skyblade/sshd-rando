...
def parse_requirement_string(req_str: str, world, area_id: int = None) -> Requirement:
    ...
    if nesting_level != 1:
        raise RequirementError(f"Extra or missing parenthesis in logic statement \"{req_str}\"")
    ...
    if len(split_req_str) == 1:
        ...
        # we can resolve them to NOTHING or IMPOSSIBLE requirements now 
            ...
            if setting_name not in settings:
                raise RequirementError(f"Setting \"{setting_name}\" is not a known setting")
        
            actual_option = world.setting(setting_name).value_index()
            compared_option = world.setting(setting_name).value_index(compared_option_str)
            if ("==" in arg and actual_option == compared_option) or ("!=" in arg and actual_option != compared_option) or (">=" in arg and actual_option >= compared_option):
                req.type = RequirementType.NOTHING
            else:
                req.type = RequirementType.IMPOSSIBLE
        # Then boolean setting checks
        elif arg in settings:
            req.type = RequirementType.NOTHING if world.setting(arg) == "on" else RequirementType.IMPOSSIBLE
        # Then area access...
    ...
    # that can currently be is a NOT requirement
        ...
        if split_req_str[0] == "not":
            ...
            raise RequirementError(f"Unrecognized 2 part logic expression {split_req_str}")
    ...
    if and_type and or_type:
        raise RequirementError(f"\"and\" & \"or\" in same nesting level when parsing {split_req_str}")
    
    # If we have neither, that's also a problem
    if not and_type and not or_type:
        raise RequirementError(f"Could not determine logical operator type from expression {split_req_str}")
...
def evaluate_requirement_at_time(req: Requirement, search, time: int, world) -> bool:
    match req.type:
        ...
        case RequirementType.OR:
            return any([evaluate_requirement_at_time(arg, search, time, world) for arg in req.args])
        
        case RequirementType.AND:
            return all([evaluate_requirement_at_time(arg, search, time, world) for arg in req.args])
        ...
        case RequirementType.GRATITUDE_CRYSTALS:
            ...
            num_single_crystals = search.owned_items[world.get_item("Gratitude Crystal")]
            num_crystal_packs = search.owned_items[world.get_item("Gratitude Crystal Pack")]
...
def evaluate_exit_requirement(search, exit_) -> int:
    ...
    connected_area_time = TOD.NONE if connected_area.id not in search.area_time else search.area_time[connected_area.id]

    # If there's no potential to spread time to the new area, then this
    # exit isn't going to be a success
    potential_time_spread = ~connected_area_time & (parent_area_time & connected_area.allowed_tod)
    ...
    for time in ALL_TODS:
        if potential_time_spread & time:
            eval_test = evaluate_requirement_at_time(exit_.requirement, search, time, exit_.world)
            if eval_test:
...
def evaluate_location_requirement(search, loc_access) -> int:
    ...
    if evaluate_requirement_at_time(loc_access.req, search, time, loc_access.area.world):
        return EvalSuccess.COMPLETE
    return EvalSuccess.NONE
