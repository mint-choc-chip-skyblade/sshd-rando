...
class Search:
    def __init__(self, search_mode_: int, worlds_: list[World], items_: list[Item] = [], world_to_search_: int = -1) -> None:
        ...
        for world in self.worlds:
            for area in world.areas.values():
                for loc_access in area.locations:
                    if not loc_access.location.is_empty() or self.search_mode == SearchMode.ALL_LOCATIONS_REACHABLE:
    ...
    def process_exits(self) -> None:
        ...
        for exit_ in self.exits_to_try:
            ...
            if exit_ in self.successful_exits or (self.world_to_search != -1 and self.world_to_search != exit_.world.id):
                continue
    ...
    def process_events(self) -> None:
        for event in self.events_to_try:
            ...
            if event.id in self.owned_events or (self.world_to_search != -1 and event.area.world.id != self.world_to_search):
                continue
    ...
    def process_locations(self, item_locations: set[LocationAccess]) -> None:
        ...
        for loc_access in item_locations:
            ...
            if loc in self.visited_locations or loc_access.area not in self.visited_areas or (self.world_to_search != -1 and world.id != self.world_to_search):
                continue
    ...
    def process_location(self, location: Location) -> None:
        ...
        if self.search_mode == SearchMode.GENERATE_PLAYTHROUGH and location.current_item.is_major_item:
        ...
        # world
        if self.search_mode in [SearchMode.GENERATE_PLAYTHROUGH, SearchMode.GAME_BEATABLE] and location.current_item.is_game_winning_item:
            if len([item for item in self.owned_items if item.is_game_winning_item]) == len(self.worlds):
    ...
    def dump_world_graph(self, world_num: int = 0, filename: str = "World"):
        ...
        with open(filename + ".gv", "w") as world_graph:
            ...
            for area_id, area in world.areas.items():
                ...
                world_graph.write(f"\t\"{area}\"[label=<{area}{tod_str}> shape=\"plain\" fontcolor={color}];\n")
                ...
                for event in area.events:
                    ...
                    world_graph.write(f"\t\"{event_name}\"[label=<{event_name}> shape=\"plain\" fontcolor={color}];")
                    world_graph.write(f"\t\"{area}\" -> \"{event_name}\"[dir=forward color={color}]")

                # Make edge connections defined by exits
                for exit_ in area.exits:
                    if exit_.connected_area != None:
                        color = "\"black\"" if exit_ in self.successful_exits else "\"red\""
                        world_graph.write(f"\t\"{area}\" -> \"{exit_.connected_area}\"[dir=forward color={color}]")
                
                # Make edge connections between areas and their locations:
                for loc_access in area.locations:
                    loc = loc_access.location
                    color = "\"black\"" if loc in self.visited_locations else "\"red\""
                    world_graph.write(f"\t\"{loc}\"[label=<{loc}:<br/>{loc.current_item}> shape=\"plain\" fontcolor={color}];")
                    world_graph.write(f"\t\"{area}\" -> \"{loc}\"[dir=forward color={color}]")
...
def generate_playthrough(worlds: list[World]) -> None:
    ...
    worlds[0].playthrough_spheres = [sphere for sphere in new_search.playthrough_spheres if len(sphere) > 0]
