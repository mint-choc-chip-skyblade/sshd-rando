...
class Setting:
    def __init__(self, name_: str = None, value_: str = None, info_: SettingInfo = None) -> None:
        self.name: str = name_
...
class SettingGet:
    ...
    def __eq__(self, value_: str) -> bool:
        if value_ not in self.setting.info.options:
            raise SettingInfoError(f"ERROR: \"{value_}\" is not a known value for {self.setting_name}")
    ...
    if len(settings_info_map) == 0:
        ...
        with open(path, "r") as settings_file:
            ...
            for setting_node in settings_yaml:
                ...
                for field in ["name", "default_option", "pretty_name", "pretty_options", "options"]:
                    if field not in setting_node:
                        raise SettingInfoError(f"Setting \"{setting_node['name']}\" is missing required field \"{field}\"")
                ...
                for pretty_option in setting_node["pretty_options"]:
                    ...
                    if '-' in pretty_option:
                        lower_bound, upper_bound = [int(bound) for bound in pretty_option.split('-')]
                        pretty_options.extend(str(o) for o in list(range(lower_bound, upper_bound + 1)))
                    else:
                    ...
                    for option_name, description in option.items():
                        ...
                        for op in option_names:
                            ...
                            if '-' in op:
                                ...
                                options.extend(str(o) for o in list(range(lower_bound, upper_bound + 1)))
                                descriptions.extend([description] * (upper_bound - lower_bound + 1))
                            else:
                                options.append(op)
                                descriptions.append(description)

                assert len(pretty_names) == len(names)
                assert len(options) == len(descriptions) and len(options) == len(pretty_options)
                ...
                for i in range(len(names)):
                    ...
                    s.default_option = options.index(default_options[min(len(default_options) - 1, i)])
                    s.options = options
                    ...
                    if "random_range" in setting_node:
                        ...
                        for field in ["first", "last"]:
                            if field not in setting_node["random_range"]:
                                raise SettingInfoError(f"Missing field \"{field}\" in random_range for \"{s.name}\"")
